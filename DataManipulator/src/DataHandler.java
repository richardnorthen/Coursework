...
public abstract class DataHandler {
	...
	public static List<String> getMergeSortedCopy(List<String> input) {
		...
		while (i < sub1.size() && j < sub2.size()) {
			...
			if (sub1.get(i).compareTo(sub2.get(j)) <= 0) output.add(sub1.get(i++));
			else output.add(sub2.get(j++));
		}
		// add remaining values to output
		if (i >= sub1.size()) output.addAll(sub2.subList(j, sub2.size()));
		if (j >= sub2.size()) output.addAll(sub1.subList(i, sub1.size()));
		return output;
	}
}